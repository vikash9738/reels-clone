...
function UploadFile(props) {
    ...
    return (
        ...
        //     }
        // </div>
        <div style={{ textAlign: 'center', padding: '2rem', marginTop: '6rem' }}>
        {error !== '' ? (
            <Alert severity="error" style={{ marginBottom: '0.1rem' }}>
                {error}
            </Alert>
        ) : (
            <>
                ...
                <label htmlFor="upload-input">
                    <Button
                        ...
                        component="span"
                        disabled={loading}
                        style={{ marginBottom: '2rem' ,backgroundColor:'#84817a' }}
                    >
                        <MovieIcon /> &nbsp;  Upload Video 
                    </Button>
                </label>
                {loading && <LinearProgress color="primary" style={{ width: '50%', margin: 'auto' }} />}
            </>
        )}
    </div>
    )
